...
function RenderEnv({ onClick, onDelete, canDelete }:any) {
    return (
        <div className="flex items-center">
...
function ServicesCreate() {
    ...
    function onDelete(id:number) {
        setEnv((prevEnv:any) => {
            return prevEnv.filter((item:any) => item.id !== id);
          });
    }
                ...
                <div className="space-y-10 max-w-3xl">
                    ...
                    <div>
                        ...
                        {Array.isArray(env) && env.map((item) => {
                            return <RenderEnv id={item.id} onClick={() => addEnvFile()} canDelete={true} onDelete={() => onDelete(item.id)} key={item.name} />
                        })}
                        </div>
